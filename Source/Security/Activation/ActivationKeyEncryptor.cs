...
namespace System.Security.Activation
{
    ...
    public sealed class ActivationKeyEncryptor : IDisposable
    {
        ...
        /// </exception>
        public ActivationKeyEncryptor(SymmetricAlgorithm symmetricAlgorithm, HashAlgorithm hashAlgorithm,
            params object[] environment)
        {
            ...
            {
                _encryptor =
                    symmetricAlgorithm.CreateEncryptor(deriveBytes.GetBytes(symmetricAlgorithm.KeySize / 8), seed);
            }
        }
        ...
        /// </summary>
        /// <param name="data">
        ///     Data that will be stored in the key and can be retrieved when the key is verified.
        /// </param>
        /// <returns>
        ///     The <see cref="ActivationKey"/> instance received by the current <see cref="ActivationKeyEncryptor"/> object.
        /// </returns>
        public ActivationKey Generate(params object[] data)
        {
            if (_encryptor == null || _hasher == null)
                throw new ObjectDisposedException(nameof(ActivationKeyEncryptor),
                    GetResourceString("ObjectDisposed_Generic"));

            ActivationKey activationKey = Generate(DateTime.MaxValue.Date, data);
